...
`authenticated` only, nothing for `anon`, realtime publication; `dish_versions` is APPEND-ONLY —
select + insert, no update/delete grant and no update/delete policy) and adds five NULLABLE provenance
...
3. Verify again: §1 identical · §2 four tables with RLS · §3 14 policies (four each for the
   three mutable tables, SELECT + INSERT for `dish_versions`) · §4 no `anon` row ·
