-- ============================================================================
-- Dishes, weight bridges and label-estimated products (DEC-037) — apply
-- migration 20260923090000 to PRODUCTION (project rqgoiuztphkcvbwtbxbj) from
-- the Dashboard SQL Editor, and record it in the CLI migration ledger.
-- GENERATED from supabase/migrations/20260923090000_dishes_bridges_estimated.sql.
-- Reviewed 2026-09-23 (docs/claude-runs/RUN_2026-09-23_UNIFIED_POINTS_DISHES_LABEL_ESTIMATOR_REPORT.md).
--   * CREATE TABLE IF NOT EXISTS × 4 (household RLS, authenticated-only, realtime);
--   * ADD COLUMN IF NOT EXISTS × 5 on food_entries, all nullable;
--   * no existing row, column, policy or grant is changed or removed.
-- HOW TO USE: run supabase/verify_dishes_bridges_estimated.sql (read-only) before
-- and after; the counts printed here must be identical BEFORE and AFTER.
-- ============================================================================
...
begin
  ...
  values ('20260923090000', 'dishes_bridges_estimated',
          array['create table if not exists public.estimated_products / weight_bridges / dishes / dish_versions with household RLS, grants and realtime',
                'alter table public.food_entries add column if not exists dish_id / dish_revision / estimated_product_id / consumed_weight_g / weight_source'])
...
-- AFTER (must be identical to BEFORE; the four new tables start empty)
select (select count(*) from public.households) as households,
       (select count(*) from public.profiles) as profiles,
       (select count(*) from public.foods) as foods,
       (select count(*) from public.food_entries) as food_entries,
