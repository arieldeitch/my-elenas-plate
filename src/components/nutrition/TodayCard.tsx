...
export function TodayCard({ onOpenCalendar, onOpenReview }: Props) {
  const { activeProfile, selectedDate, setSelectedDate, getDay, foods, dailyPointsBudgets } =
    useStore();
  ...
  return (
    ...
    >
      ...
      <div
        className="mt-3 flex items-center justify-between rounded-xl border border-border bg-secondary/60 px-3 py-2"
        data-testid="today-points"
      >
        <div>
          <div className="text-xs text-muted-foreground">נקודות היום</div>
          <div className="font-bold tabular-nums">
            {formatPoints(points)} / {budget}
          </div>
        </div>
        <div
          className={cn(
        ...
        >
          {remaining < 0
            ? `${formatPoints(Math.abs(remaining))} מעל`
            : `${formatPoints(remaining)} נותרו`}
        </div>
        <button
          type="button"
          onClick={() => setBudgetOpen(true)}
          aria-label="עריכת תקציב נקודות"
