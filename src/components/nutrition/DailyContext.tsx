  ...
  const tiles = [
    {
      key: "weight",
      label: "שקילה",
      value: latest ? `${formatNumber(latest.weightKg)} ק״ג` : "לא תועד",
      sub: latest ? "שקילה אחרונה" : "הוספת שקילה",
      icon: Scale,
    ...
    },
    {
      ...
      label: "אימון",
      value:
        day.workout?.performed === true
          ? (day.workout.type ?? "בוצע")
          : day.workout?.performed === false
            ? "לא בוצע"
            : "לא תועד",
      sub: "ליום הנבחר",
      icon: Dumbbell,
      tone: "bg-primary-soft text-primary",
      action: () => setEditor(editor === "workout" ? null : "workout"),
    },
    {
      key: "fasting",
      label: "צום",
      value: day.fasting
        ? `${calcFastingHours(day.fasting.start, day.fasting.end)} שעות`
        : "לא תועד",
      sub: day.fasting ? `${day.fasting.start}–${day.fasting.end}` : "ליום הנבחר",
      icon: Hourglass,
      tone: "bg-info-soft text-info",
      action: () => setEditor(editor === "fasting" ? null : "fasting"),
    },
    {
      key: "steps",
      label: "צעדים",
      value:
        steps?.steps != null
          ...
          : steps?.completed
            ? "בוצע"
            : "לא תועד",
      sub: `יעד ${(steps?.goal ?? store.stepGoal).toLocaleString("he-IL")}`,
      icon: Footprints,
      tone: "bg-info-soft text-info",
      action: () => setEditor(editor === "steps" ? null : "steps"),
    },
  ];
  return (
    <section aria-labelledby="daily-context-title" className="mt-5">
      <h2 id="daily-context-title" className="mb-3 px-1 text-[15px] font-semibold">
        היום שלי
      </h2>
      <div className="daily-context-grid">
        {tiles.map(({ key, label, value, sub, icon: Icon, tone, action }) => (
          ...
          >
            ...
            <span className="mt-1 max-w-full truncate text-sm font-bold">{value}</span>
            <span className="mt-0.5 max-w-full truncate text-[11px] text-muted-foreground">
              {sub}
            </span>
          </button>
        ))}
