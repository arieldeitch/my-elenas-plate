...
import { calcFatMass, calcWeightDelta, latestAndPrevious } from "@/lib/weight";
...
export function WeightBanner({ onOpen }: Props) {
  ...
  const { latest, previous: prev } = latestAndPrevious(weighIns);
...
function DeltaLine({
  ...
  if (prev == null) {
    ...
    const diff = Math.round((calcWeightDelta(curr, prev) ?? 0) * 10) / 10;
    ...
    bits.push(
      ...
      >
        ...
        {sign}
        {formatNumber(Math.abs(diff))} ק״ג מהשקילה הקודמת
      </span>,
    );
  }
  const fatMass = calcFatMass(curr, bodyFatPct);
  if (fatMass != null) {
    bits.push(
      <span key="fm" className="text-xs text-muted-foreground">
        מסת שומן: {formatNumber(Math.round(fatMass * 10) / 10)} ק״ג
      </span>,
    );
  }
