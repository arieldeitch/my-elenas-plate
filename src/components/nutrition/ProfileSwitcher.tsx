...
export function ProfileSwitcher() {
  const { activeProfile, setActiveProfile, deviceProfile, openDeviceChooser } = useStore();
  const activeIndex = PROFILES.findIndex((p) => p.id === activeProfile);
  const deviceName = PROFILES.find((p) => p.id === deviceProfile)?.name;
  ...
  return (
    <div className="flex flex-col items-start gap-1">
      ...
      {/* Device default (M1 Phase B): explicit, per-device, changeable. */}
      <button
        type="button"
      ...
      >
        ...
        <span>
          {deviceName ? `ברירת מחדל במכשיר הזה: ${deviceName}` : "לא נבחר פרופיל למכשיר הזה"}
        </span>
      </button>
    </div>
  );
